...
# Remote I/O
The user can change the expansion config in this block, see `config_file`. The other input files are specified in the `config_file`. 
```{r remote-io}
setwd(getwd())
...
# Parameters
The parameters for the expansion:
1. `max_iteration` - The number of iterations to fit the data (50 should be more than enough)
2. `scaling_factor_low` - The minimum allowable weight is the mean expected weight (see `avgweight` in the `preparation` block below) divided by this factor
3. `scaling_factor_high` - The maximum allowable weight is the mean expected weight multiplied by this factor
4. `departing_only` - Set to `TRUE` for the departing only expansion and `FALSE` for the departing plus arriving expansion
```{r parameters}
...
# Reads
Read the data from disk, setting the data types for the key variables. 
```{r data-reads}
...
For each row in the survey table, this method determines which of the controls are incident (i.e., relevant)
to the row. At least one control should be relevant to each control. 
...
Initializes the relaxation controls used in the fitting algorithms to 1.0. 
...
Uses the Netwon-Raphson method to fit the controls. For each control, it iteratively adjusts the weight to match the control.
The importance of each weight is use to dampen the correction. 
```{r method-expansion}
...
Helper method to print out statistics for each round of expansion. 
...
# Preparation
Prepare the data files for the expansion, setting the incidence values for each row and initializing the relaxation factors.
```{r preparation}
...
# Expansion
Run the expansion, which calls the fsitting algorithm `max_iteration` times. There is no assessment of fit, it just runs through `max_iteration`. Statistics for
each iteration are logged, so you can see how well the algorithm is doing. Runtime should be about 15 seconds on a laptop. 
```{r expansion}
...
# Resolution
Prepare the results to be written to disk. 
```{r resolution}
